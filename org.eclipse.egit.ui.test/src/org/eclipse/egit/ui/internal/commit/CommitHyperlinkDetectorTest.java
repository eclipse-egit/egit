...
public class CommitHyperlinkDetectorTest {
	...
	public void testMultiLine() {
		...
		IHyperlink[] hyperlinks = detectHyperlinks(0,
				textViewer.getDocument().getLength());
		assertEquals(1, hyperlinks.length);
		assertEquals(EXAMPLE_ID, hyperlinks[0].getHyperlinkText());
		assertEquals(new Region(21, EXAMPLE_ID.length()),
	...
	public void testGerritIdWithinText() {
		...
		IHyperlink[] hyperlinks = detectHyperlinks(5,
				textViewer.getDocument().getLength());
		assertNull(hyperlinks);
	}
	...
	private IHyperlink[] detectHyperlinks(int offset, int length) {
		return detector.detectHyperlinks(textViewer, new Region(offset, length),
				false);
	}
