...
public class StageUnstageActionTest extends LocalRepositoryTestCase {
	...
	public void testCompareIndexWithHeadEnablement() throws Exception {
		...
		// Add it to the index
		SWTBotTree projectExplorerTree = TestUtil.getExplorerTree();
		SWTBotTreeItem projectAItem = util.getProjectItems(projectExplorerTree, PROJ_A)[0];
		SWTBotTreeItem folderItem = TestUtil.expandAndWaitFor(projectAItem, FOLDER);
		SWTBotTreeItem fileItem = TestUtil.expandAndWaitFor(folderItem, FILE1);
		fileItem.select();
