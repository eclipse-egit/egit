 ...
 * secure store. A login popup is sent if credentials are not available.
 */
public class EGitCredentialsProvider extends CredentialsProvider {
	...
	public boolean get(final URIish uri, CredentialItem... items)
		...
		EGitCredentials credentials = getCredentialsFromSecureStore(uri);
	...
	private EGitCredentials getCredentialsFromUser(final URIish uri) {
		final AtomicReference<EGitCredentials> aRef = new AtomicReference<EGitCredentials>(
				null);
	...
	private EGitCredentials getCredentialsFromSecureStore(final URIish uri) {
