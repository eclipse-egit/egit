		...
		if (diffCont != null && diffCont.hasChildren()) {
			...
			for (IDiffElement diffElement : difContChilds) {
				...
				case Differencer.CONFLICTING:
					/*
					 * DiffNode node = (DiffNode) diffElement; // get the left
					 * resource and revisions FileRevisionTypedElement
					 * leftRevision = (ResourceEditableRevision) node
					 * .getLeft(); IResource leftResource =
					 * ((ResourceEditableRevision) node
					 * .getLeft()).getResource(); FileRevisionTypedElement
					 * rightRevision = (FileRevisionTypedElement) node
					 * .getRight(); FileRevisionTypedElement baseRevision =
					 * (FileRevisionTypedElement) node .getAncestor(); // get
					 * file names String mergedAbsoluteFilePath = null; String
					 * mergedRelativeFilePath = null; String mergedFileName =
					 * null; String localAbsoluteFilePath = null; String
					 * remoteAbsoluteFilePath = null; String
					 * baseAbsoluteFilePath = null; String mergeCmd = null;
					 * String mergeBaseLessCmd = null; boolean prompt = false;
					 * boolean trustExitCode = true; // boolean keepBackup =
					 * false; // TODO boolean writeToTemp = false; boolean
					 * keepTemporaries = false; File mergedDirPath = null; File
					 * tempDirPath = null; File workDirPath = null; Repository
					 * repository = null; if (leftResource != null) {
					 * mergedAbsoluteFilePath = leftResource.getRawLocation()
					 * .toOSString(); mergedFileName = leftResource.getName();
					 * mergedDirPath = leftResource.getRawLocation().toFile()
					 * .getParentFile(); System.out.println("file: "
					 * //$NON-NLS-1$ + mergedAbsoluteFilePath); } if
					 * (mergedAbsoluteFilePath != null) { IPath[] paths = new
					 * Path[1]; paths[0] = new Path(mergedAbsoluteFilePath);
					 * Map<Repository, Collection<String>> pathsByRepository =
					 * ResourceUtil
					 * .splitPathsByRepository(Arrays.asList(paths));
					 * Set<Repository> repos = pathsByRepository.keySet(); if
					 * (repos.size() == 1) { repository =
					 * repos.iterator().next(); } if (repository != null) {
					 * workDirPath = repository.getWorkTree(); } } if
					 * (mergedAbsoluteFilePath != null && leftRevision != null
					 * && rightRevision != null) { // get the relative project
					 * path from left revision here mergedRelativeFilePath =
					 * leftRevision.getPath(); // get the tool ITool tool =
					 * GitPreferenceRoot.getExternalMergeTool(); if (tool !=
					 * null) { // get the command mergeCmd = tool.getCommand();
					 * // empty or index = 0 // is the default // command (with
					 * // $BASE support) mergeBaseLessCmd = tool.getCommand(1);
					 * // index = 1 // is the // alternative // command //
					 * (without // $BASE // support) // get other attribute
					 * values prompt = GitPreferenceRoot
					 * .getExternalMergeToolAttributeValueBoolean(
					 * tool.getName(), "prompt"); //$NON-NLS-1$ trustExitCode =
					 * GitPreferenceRoot
					 * .getExternalMergeToolAttributeValueBoolean(
					 ...
					 * first check if we should ask user if (prompt) { int
					 * response = ToolsUtils
					 * .askUserAboutToolExecution("mergetool", //$NON-NLS-1$
					 * "Merging file: " //$NON-NLS-1$ + mergedRelativeFilePath +
					 * "\n\nLaunch '" //$NON-NLS-1$ + tool.getName() + "' ?");
					 * //$NON-NLS-1$ if (response == SWT.NO) { break; } else if
					 * (response == SWT.CANCEL) { return; } } // check if temp
					 * dir should be created if (writeToTemp) { tempDirPath =
					 * ToolsUtils .createDirectoryForTempFiles(); mergedDirPath
					 * = tempDirPath; } localAbsoluteFilePath =
					 * ToolsUtils.loadToTempFile( mergedDirPath, mergedFileName,
					 * "LOCAL", //$NON-NLS-1$ leftRevision, writeToTemp);
					 * remoteAbsoluteFilePath = ToolsUtils.loadToTempFile(
					 * mergedDirPath, mergedFileName, "REMOTE", //$NON-NLS-1$
					 * rightRevision, writeToTemp); baseAbsoluteFilePath =
					 * ToolsUtils.loadToTempFile( mergedDirPath, mergedFileName,
					 * "BASE", //$NON-NLS-1$ baseRevision, writeToTemp); } } //
					 * execute int exitCode = -1; try { exitCode =
					 * ToolsUtils.executeTool(workDirPath,
					 ...
					 * mergeBaseLessCmd, tempDirPath); } catch (IOException |
					 * InterruptedException e) { e.printStackTrace();
					 * ToolsUtils.informUserAboutError("mergetool - error",
					 * //$NON-NLS-1$ e.getMessage() + "\n\nMerge aborted!");
					 * //$NON-NLS-1$ return; // abort the merge process }
					 * finally { System.out.println("exitCode: " //$NON-NLS-1$ +
					 * Integer.toString(exitCode)); // delete temp if
					 * (tempDirPath != null && !keepTemporaries) {
					 * ToolsUtils.deleteDirectoryForTempFiles(tempDirPath); } }
					 * // add file to stage if successfully merged boolean
					 * addFile = false; if (trustExitCode) {
					 * System.out.println("trustExitCode: true"); //$NON-NLS-1$
					 * if (exitCode == 0) { addFile = true; } } else {
					 ...
					 * "Merging file: " //$NON-NLS-1$ + mergedRelativeFilePath +
					 * "\n\nWas the merge successful?"); //$NON-NLS-1$ if
					 * (response == SWT.YES) { addFile = true; } else if
					 * (response == SWT.CANCEL) { return; // abort the merge
					 * process } } if (repository != null) { if (addFile &&
					 * GitPreferenceRoot.autoAddToIndex() &&
					 * mergedAbsoluteFilePath != null) { Git git = new
					 * Git(repository); try {
					 * git.add().addFilepattern(mergedRelativeFilePath) .call();
					 * } catch (GitAPIException e) { e.printStackTrace(); }
					 * git.close(); } repository.close(); } break;
					 */
				}
			}
		}
