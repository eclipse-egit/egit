...
public class MergeActionHandler extends RepositoryActionHandler {
			...
			Job job = new Job(jobname) {
				...
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						getShell(event).getDisplay().asyncExec(new Runnable() {
							public void run() {
								Activator
										.handleError(
	...
	private boolean canMerge(final Repository repository, ExecutionEvent event) {
		...
		if (message != null) {
			MessageDialog.openError(getShell(event), UIText.MergeAction_CannotMerge,
					message);
		}
		return (message == null);
	}
	...
	public boolean isEnabled() {
		...
		Repository repository = getRepository(false, null);
		if (repository == null)
			enabled = false;
		return enabled;
	}
