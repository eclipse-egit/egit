...
public class DiscardChangesAction extends RepositoryAction{
	...
	private void performDiscardChanges() {
		for (final IResource res : getSelectedResources()) {
			...
			StringBuilder resPath = new StringBuilder(res.getLocation().toOSString());
			resPath.delete(0, workingDirPath.length() + 1);

			try {
				Entry e = repository.getIndex().getEntry(resPath.toString());
