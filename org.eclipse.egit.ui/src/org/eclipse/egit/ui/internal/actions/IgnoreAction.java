				...
				try {
					for (IResource resource : resources) {
						...
						if (!Team.isIgnoredHint(resource)) {
							...
							// TODO What is the character set and new-line
							// convention?
							if (gitignore.exists()) {
								gitignore.appendContents(
										new ByteArrayInputStream(entry
												.getBytes(Constants.CHARACTER_ENCODING)), true,
										true, monitor);
							} else {
					...
					throw new CoreException(
							new Status(
									IStatus.ERROR,
									"org.eclipse.egit.ui", "Unable to ignore resources", e)); //$NON-NLS-1$
				}
