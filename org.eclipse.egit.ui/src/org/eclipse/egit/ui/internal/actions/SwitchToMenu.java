...
public class SwitchToMenu extends ContributionItem implements
	...
	private static final int MAX_NUM_MENU_ENTRIES = 50;
	...
	public void fill(Menu menu, int index) {
		...
		try {
			...
			localBranches = mapping.getRepository().getRefDatabase().getRefs(
					Constants.R_HEADS);
			int itemCount = 0;
			for (final Entry<String, Ref> entry : localBranches.entrySet()) {
				itemCount++;
				...
				item.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						new BranchOperationUI(mapping.getRepository(), entry
								.getValue().getName()).start();
					}
				});
			}
			...
			others.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					BranchSelectionDialog dlg = new BranchSelectionDialog(
							shell, mapping.getRepository());
					if (dlg.open() == Window.OK) {
						new BranchOperationUI(mapping.getRepository(), dlg
								.getRefName()).start();
					}
				}
			});
