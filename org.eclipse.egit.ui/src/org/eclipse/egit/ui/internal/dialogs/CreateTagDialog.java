...
public class CreateTagDialog extends TitleAreaDialog {
	...
	private Button lightweightButton;
	...
	protected void buttonPressed(int buttonId) {
		if (buttonId == CLEAR_ID) {
			...
			lightweightButton.setSelection(false);
			...
			annotated = !lightweightButton.getSelection();
			okPressed();
		} else {
	...
	private void validateInput() {
		...
		boolean isLightWeight = lightweightButton.getSelection();
		Control button = getButton(IDialogConstants.OK_ID);
		if (button != null) {
		...
		boolean editable = (existingTagSelected
				&& !overwriteButton.getSelection()) || isLightWeight;
		tagMessageText.getTextWidget().setEditable(editable);
