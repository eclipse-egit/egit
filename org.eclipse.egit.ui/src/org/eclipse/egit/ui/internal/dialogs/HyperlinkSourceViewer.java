...
public class HyperlinkSourceViewer extends SourceViewer {
	 ...
	 * SWT.NONE) if getHyperlinkStateMask(ISourceViewer) returns SWT.NONE.
	 */
	public static class Configuration extends TextSourceViewerConfiguration {
		...
		public IHyperlinkDetector[] getHyperlinkDetectors(
			...
			if (detectors != null && detectors.length > 0) {
				int mask = getHyperlinkStateMask(sourceViewer);
				int defaultMask = getConfiguredDefaultMask();
				if (mask == SWT.NONE) {
					// Duplicate them all with a detector that is active on
					// SWT.NONE
					IHyperlinkDetector[] newDetectors = new IHyperlinkDetector[detectors.length
							* 2];
					int j = 0;
					for (int i = 0; i < detectors.length; i++) {
						IHyperlinkDetector original = detectors[i];
						if (original instanceof IHyperlinkDetectorExtension2) {
							mask = ((IHyperlinkDetectorExtension2) original)
									.getStateMask();
							if (mask == -1) {
		...
		/**
		 * Actually collects the {@link IHyperlinkDetector}s for the given
		 * {@link ISourceViewer}.
		 *
		 * @param sourceViewer
		 *            to getthe detectors for
	...
	private static abstract class InternalHyperlinkDetector
		...
		private InternalHyperlinkDetector(IHyperlinkDetector delegate) {
	...
	private static class FixedMaskHyperlinkDetector
		...
		private FixedMaskHyperlinkDetector(IHyperlinkDetector delegate,
				int mask) {
			super(delegate);
			this.mask = mask;
		}
	...
	// Unfortunately, the preference for
	// AbstractTextEditor.PREFERENCE_HYPERLINK_KEY_MODIFIER_MASK is
	// not recomputed when the user changes preferences. Therefore, we have to
