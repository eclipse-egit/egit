 ...
 *
 */
public class AdvancedBranchSelectionDialog extends
	 ...
	 * @param settings
	 */
	public AdvancedBranchSelectionDialog(Shell parentShell, Repository repo,
			int settings) {
		super(parentShell, repo, settings);
		try {
	...
	protected String getTitle() {
		return "Select a branch or work with branches"; //$NON-NLS-1$ TODO
	}
				...
				if (new WizardDialog(getShell(), wiz).open() == Window.OK) {
					...
					try {
						...
						if (repo.getBranch().equals(newRefName))
							...
							AdvancedBranchSelectionDialog.this
									.okPressed();
					} catch (Throwable e1) {
						reportError(
								e1,
