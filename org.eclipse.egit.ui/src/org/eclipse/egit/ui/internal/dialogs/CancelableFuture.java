 ...
 * perform long tasks. The {@link CancelableFuture#get() get()} method blocks
 * until the result is available or the future is canceled. The first call to
 * {@link CancelableFuture#get() get()} starts a background job running the
 * {@link CancelableFuture#run(IProgressMonitor) run()} operation, which is
 * supposed to call {@link #set(Object) set()} to set its result. Pre-fetching
 * is possible by calling {@link CancelableFuture#start()} directly.
 * <p>
 ...
 * it's been canceled. A {@link CancelableFuture} accepts a result even after
 * having been canceled, so a subsequent get() may get that result. In both,
 * cancellation wakes up all waiting calls to get().</li>
 * <li>A {@link CancelableFuture} automatically runs the task in a background
 * job, whereas a FutureTask would need an extra ExecutorService to
 * asynchronously run its task.</li>
 * <li>This class uses Eclipse jobs instead of Threads directly.</li>
 * </ul>
 *
 * @param <V>
 *            Type of the result
...
public abstract class CancelableFuture<V> {
	...
	public static enum CancelMode {
		 ...
		 * InterruptedException or return a result if the job terminated in the
		 * meantime.
		 */
		ABANDON,
		/**
		 * Tries to cancel the job, and if that doesn't succeed immediately,
		 * interrupts the job's thread. Outstanding calls to get() will be woken
		 * up and may throw InterruptedException or return a result if the job
	 ...
	 * cancellation, which may or may not terminated the job (it may decide not
