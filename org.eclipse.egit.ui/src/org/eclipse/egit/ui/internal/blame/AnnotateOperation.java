...
public class AnnotateOperation implements IEGitOperation {
	...
	public void execute(IProgressMonitor monitor) throws CoreException {
		...
		command.setFilePath(mapping.getRepoRelativePath(file));
		final List<Revision> allRevisions = command.call();
		if (allRevisions.isEmpty())
			return;
		Map<RevCommit, AnnotationRevision> revisions = new HashMap<RevCommit, AnnotationRevision>();
		...
		for (Line line : latestRevision.getLines()) {
			RevCommit commit = line.getStart();
			AnnotationRevision revision = revisions.get(commit);
			if (revision == null) {
