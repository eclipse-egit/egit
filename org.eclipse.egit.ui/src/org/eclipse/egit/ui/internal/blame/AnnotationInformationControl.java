...
public class AnnotationInformationControl extends AbstractInformationControl
	...
	private AnnotationRevision revision;

	private ScrolledComposite scrolls;

	private Composite displayArea;

	private Link commitLink;
	 ...
	 * @param creator
	 */
	public AnnotationInformationControl(Shell parentShell, boolean isResizable,
	...
	protected void createContent(Composite parent) {
		...
		commitLink = new Link(displayArea, SWT.NONE);
		commitLink.addSelectionListener(new SelectionAdapter() {

			public void widgetSelected(SelectionEvent e) {
				try {
					getShell().dispose();
					CommitEditor.open(new RepositoryCommit(revision
							.getRepository(), revision.getRevision()
							.getCommit()));
	...
	public void setInput(Object input) {
		this.revision = (AnnotationRevision) input;

		RevCommit commit = this.revision.getRevision().getCommit();

		commitLink.setText(MessageFormat.format(
				UIText.AnnotationInformationControl_Commit, commit.name()));

		PersonIdent author = commit.getAuthorIdent();
		if (author != null) {
			setControlVisible(authorLabel, true);
			authorLabel.setText(MessageFormat.format(
					UIText.AnnotationInformationControl_Author,
		...
		if (committer != null && !committer.equals(author)) {
			...
			committerLabel.setText(MessageFormat.format(
					UIText.AnnotationInformationControl_Committer,
