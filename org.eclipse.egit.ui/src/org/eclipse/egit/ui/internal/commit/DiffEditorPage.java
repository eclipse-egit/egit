...
public class DiffEditorPage extends TextEditor
	...
	private static class ThemePreferenceStore extends PreferenceStore {
		...
		private void initColors() {
			...
			// usually are already rather pale, let's saturate them more,
			// otherwise the annotations will be barely visible.
			RGB rgb = saturate(
					currentColors.getRGB(THEME_DiffAddBackgroundColor), 4.0);
			...
			rgb = saturate(
					currentColors.getRGB(THEME_DiffRemoveBackgroundColor), 4.0);
		...
		private RGB saturate(RGB rgb, double saturation) {
