...
/**
 * A component that periodically fetches from all remotes of all tracked
 * repositories in the background. Controlled by the
 * {@link UIPreferences#AUTO_FETCH} and
 * {@link UIPreferences#AUTO_FETCH_INTERVAL} preferences.
 */
	...
	static class AutoFetchJob extends Job implements IPropertyChangeListener {
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			for (Repository repo : repos) {
				...
				try {
					List<RemoteConfig> remotes = RemoteConfig
		...
		private void updateFetchInterval() {
			IPreferenceStore store = Activator.getDefault()
					.getPreferenceStore();
			boolean enabled = store.getBoolean(UIPreferences.AUTO_FETCH);
			interval = enabled
