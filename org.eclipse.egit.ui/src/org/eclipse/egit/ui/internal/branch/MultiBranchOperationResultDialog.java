...
public class MultiBranchOperationResultDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		table.setSize(300, 1200);
		// int linesToShow = Math.min(Math.max(results.size(), 5), 15);
		// int heightHint = table.getItemHeight() * linesToShow + 500;
		// GridDataFactory.fillDefaults().grab(true, true).hint(1800,
		// heightHint)
		// .applyTo(table);
		// GridDataFactory.fillDefaults().grab(true, true).minSize(800, 800)
		// .applyTo(table);
		// repository
		...
		tc.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				...
				if (item.getValue().getStatus() == Status.OK)
					return UIText.MultiBranchOperationResultDialog_OkStatus;
		...
		String title = NLS.bind(
				UIText.MultiBranchOperationResultDialog_DialogTitle,
				"SITA_TAXI"); //$NON-NLS-1$
		setTitle(title);
		setErrorMessage(
