 ...
 *     Mathias Kinzler <mathias.kinzler@sap.com> - copied from
 *                  org.eclipse.ui.internal.part.StatusPart and added
 *                  a changeReason method
 *******************************************************************************/
...
/**
 * An extended version of org.eclipse.ui.internal.part.StatusPart
 */
class EGitStatusPart {
	 ...
	 * @param reason_
	 */
	public EGitStatusPart(final Composite parent, IStatus reason_) {
		...
		detailsButton.setLayoutData(new GridData(SWT.BEGINNING, SWT.FILL,
				false, false));
	...
	public void changeReason(IStatus status) {
		this.reason = status;
		this.text.setText(reason.getMessage());
		detailsButton.setVisible(reason.getException() != null);
		updateDetailsText();
	...
	private void updateDetailsText() {
		...
		if (showingDetails) {
			...
			Text detailsText = new Text(detailsArea, SWT.BORDER | SWT.H_SCROLL
	...
	private void createShowLogButton(Composite parent) {
		IViewDescriptor descriptor = PlatformUI.getWorkbench()
				.getViewRegistry().find(LOG_VIEW_ID);
		if (descriptor == null) {
			return;
		}
		...
		button.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					Activator.handleError(ce.getMessage(), ce, false);
				}
			}
		});
		button.setImage(descriptor.getImageDescriptor().createImage());
		button.setToolTipText(UIText.EGitStatusPart_ShowErrorLogButton);
	}
}
