...
public abstract class ControlLabelProvider extends ColumnLabelProvider {
	...
	/**
	 * Creates a new {@link ControlLabelProvider}.
	 *
	 * @param columnIndex
	 *            of the column the label provider belongs to
	 */
	public ControlLabelProvider(int columnIndex) {
	...
	private class Editor extends TableEditor {
		...
		public void layout() {
			if (Util.isGtk() && SWT.getVersion() <= 4924) {
				...
				// SWT versions < 4924r7.
				Rectangle rect = getItem().getBounds();
				if (table.getHeaderVisible()) {
					Control editor = getEditor();
					if (editor != null && !editor.isDisposed()) {
						editor.setVisible(rect.y >= table.getHeaderHeight());
					}
				}
			}
			super.layout();
