...
public class BranchNameNormalizer {
	...
	public BranchNameNormalizer(Text text, String tooltipText) {
		...
		decorator.hide();
		proposer = new ContentProposalAdapter(text, new TextContentAdapter(),
				(c, p) -> {
					if (c.isEmpty() || Repository
							.isValidRefName(Constants.R_HEADS + c)) {
						return null;
					}
					return new ContentProposal[] { new ContentProposal(
							Repository.normalizeBranchName(c)) };
	...
	/**
	 * Change the visibility of the decorator.
	 *
	 * @param visible
