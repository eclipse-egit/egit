 ...
 *     Ian Pun - reimplemented to work with Git Cloning DND using MarketplaceDropAdapter.java
 *******************************************************************************/
...
/**
 * Adapter to listen for any Drag and Drop operations that are GitHub based. If
 * it goes through the parser correctly, a Clone Git Repo wizard will appear and
 * be populated.
 *
 */
...
public class GitCloneDropAdapter implements IStartup {
	...
	protected void proceedClone(String url) {
		GitUrlHandler.triggerClone(url);
	}
		...
		private boolean dropTargetIsValid(DropTargetEvent e, boolean isDrop) {
			if (URLTransfer.getInstance().isSupportedType(e.currentDataType)) {
				...
				if (isDrop || Util.isWindows()) {
					...
					if (!GitUrlHandler.isPotentialSolution(url)) {
						return false;
					}
				}
				return true;
