...
public class MergeStrategyHelper {
	...
	private void addMergeStrategyRow(MergeStrategyInfo info) {
		...
		btnStrategy.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				selectedStrategy = (MergeStrategyInfo) event.widget.getData();
			}
		});
		...
		GridData gd_lblDesc = new GridData(SWT.LEFT, SWT.CENTER, false, false);
		...
		GridData gd_lblProvidedBy = new GridData(SWT.LEFT, SWT.CENTER, false,
				false);
		gd_lblProvidedBy.widthHint = 150;
		...
		GridData gd_helpBtn = new GridData(SWT.CENTER, SWT.CENTER, false, false);
