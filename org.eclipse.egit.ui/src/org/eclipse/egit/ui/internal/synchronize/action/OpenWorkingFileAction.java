...
/**
 * Action for opening an editor on the currently selected git working
 * file.
 *
 * <p>
...
public class OpenWorkingFileAction extends SelectionListenerAction {
    ...
    /**
     * Creates a new action that will open editors on the selected
     * working file resources.
     *
        ...
        try {
        	...
        	IDE.openEditor(getWorkbenchPage(), file, activate);
        	 ...
        	 ErrorDialog.openError(getWorkbenchPage().getWorkbenchWindow()
                 .getShell(),
                 UIText.OpenWorkingFileAction_openWorkingFileShellTitle,
                 e.getMessage(), e.getStatus());
        }
    }

    /**
     * Opens external file, the editor that is used is based on
     * best guess from file name.
     *
