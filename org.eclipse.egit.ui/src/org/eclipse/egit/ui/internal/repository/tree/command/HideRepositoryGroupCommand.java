...
/**
 * mark repository groups as hidden
 */
public class HideRepositoryGroupCommand
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		List<RepositoryGroup> groupsToHide = new ArrayList<>();
		List<RepositoryTreeNode> elements = getSelectedNodes();
		for (Object element : elements) {
			if (element instanceof RepositoryGroupNode) {
				groupsToHide.add(((RepositoryGroupNode) element).getGroup());
			}
		}
		if (!groupsToHide.isEmpty()) {
			RepositoryGroups groups = new RepositoryGroups();
			groups.setHidden(groupsToHide, true);
