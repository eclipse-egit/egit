...
public class DeleteBranchOnCommitHandler extends AbstractHistoryCommandHandler {
	...
	private static final class BranchMessageDialog extends MessageDialog {
		...
		protected Control createCustomArea(Composite parent) {
			...
			GridDataFactory.fillDefaults().grab(true, true).applyTo(branchesList.getTable());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		final Repository repository = getPage().getInputInternal()
				.getRepository();

		final String head;
		try {
			head = repository.getFullBranch();
		} catch (IOException e) {
	...
	public boolean isEnabled() {
		...
		String head;
		try {
			head = page.getInputInternal().getRepository().getFullBranch();
		} catch (IOException e) {
			head = null;
		}
		...
		if (!(o instanceof PlotCommit))
			return false;
		PlotCommit commit = (PlotCommit) o;
		int refCount = commit.getRefCount();
		for (int i = 0; i < refCount; i++) {
			String refName = commit.getRef(i).getName();
