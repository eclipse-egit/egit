...
class FileDiff {
	...
	/**
	 * Creates a textual diff.
	 *
	 * So far this works only in case of one parent commit.
	 * @param d
	 * @param db
	 * @param diffFmt
	 * @throws IOException
	...
	public void outputDiff(final StringBuilder d,
			final Repository db, final DiffFormatter diffFmt) throws IOException {
		if (!(blobs.length == 2))
		...
		if(id1.equals(ObjectId.zeroId()))
			...
			d.append("--- a").append(IPath.SEPARATOR).append(path).append("\n"); //$NON-NLS-1$

		if(id2.equals(ObjectId.zeroId()))
			d.append("+++ /dev/null\n"); //$NON-NLS-1$
		else
			d.append("+++ b").append(IPath.SEPARATOR).append(path).append("\n"); //$NON-NLS-1$
