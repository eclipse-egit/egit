...
/**
 * Allows to select a single commit TODO localize
 */
public class CommitSelectionDialog extends TitleAreaDialog {
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText("Select a commit"); //$NON-NLS-1$
	}
	...
	public void create() {
		...
		try {
			PlatformUI.getWorkbench().getProgressService().run(true, true,
					new IRunnableWithProgress() {
						public void run(IProgressMonitor monitor)
							...
							try {
								monitor.beginTask("Building commit list", //$NON-NLS-1$
			...
			setMessage(
					"The commit list may be incomplete", IMessageProvider.WARNING); //$NON-NLS-1$
		}
	}
	...
	private void updateUi() {
		setTitle(NLS.bind("{0} commits in Repository {1}", Integer //$NON-NLS-1$
				.valueOf(allCommits.size()), repository.getDirectory()
				.toString()));
		setMessage("Please select a commit from the list"); //$NON-NLS-1$
