	...
	private StagingFolderEntry[] getFolders() {
		if (folders == null) {
			...
			for (StagingEntry stagingEntry : content) {
				IResource parent = stagingEntry.getFile();

				if (parent == null
						|| (parent.getParent() != null
						&& parent.getParent().getLocation().toOSString()
								.equals(workTreePath))) {
				...
				while (parent != null
					...
					if (parent.getParent() == null
						...
						rootFolderList.add(new StagingFolderEntry(file));
						resourceList.add(parent);
					}
					parent = parent.getParent();
					if (parent != null) {
						File file = new File(parent.getLocation().toOSString());
						folderList.add(new StagingFolderEntry(file));
						resourceList.add(parent);
					}
				}
			}
