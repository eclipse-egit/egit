...
/**
 * Label provider for {@link GitXStagedNode} objects
 */
public class GitXStagingViewLabelProvider extends BaseLabelProvider implements
		ITableLabelProvider {
	...
	public Image getColumnImage(Object element, int columnIndex) {
		if (columnIndex == 0) {
			final GitXStagedNode c = (GitXStagedNode) element;
			switch (c.getState()) {
			case ADDED:
				return getDecoratedImage(getEditorImage(c),
						UIIcons.OVR_STAGED_ADD);
			case CHANGED:
				return getDecoratedImage(getEditorImage(c), UIIcons.OVR_MODIFIED);
			case REMOVED:
				return getDecoratedImage(getEditorImage(c),
	...
	private Image getEditorImage(GitXStagedNode diff) {
		Image image = DEFAULT;
