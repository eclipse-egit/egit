...
public class ContainerTreeIteratorResourceFilterTest extends GitTestCase {
	...
	public void simpleNonInheritableFilter() throws Exception {
		...
		testUtils.addFileToProject(p, "unfiltered.txt", "");
		assertTrue("IFile should exist before filtering.", filtered.exists());

		createFilter(p, EXCLUDE_ALL | FILES, "1.0-name-matches-false-false-filtered.txt");
	...
	public void simpleNonInheritableFolderFilter() throws Exception {
		...
		testUtils.addFileToProject(p, "folder2/unfiltered.txt", "");

		createFilter(p, EXCLUDE_ALL | FOLDERS, "1.0-name-matches-false-false-folder");
		assertFalse("IFile should no longer exist after filtering.", filtered.exists());
	...
	public void inheritableFilter() throws Exception {
		...
		testUtils.addFileToProject(p, "folder1/folder2/unfiltered.txt", "");

		createFilter(p, EXCLUDE_ALL | FILES | INHERITABLE, "1.0-name-matches-false-false-filtered.txt");
	...
	public void directlyNestedProject() throws Exception {
		...
		createFilter(p, EXCLUDE_ALL | FOLDERS, "1.0-name-matches-false-false-Project-2");
	...
	public void nestedProject() throws Exception {
		...
		createFilter(p.getFolder("folder1"), EXCLUDE_ALL | FOLDERS,
				"1.0-projectRelativePath-matches-false-false-folder1/subfolder");
	...
	private static void createFilter(IContainer container, int type, String multiFilterArguments) throws CoreException {
		FileInfoMatcherDescription matcherDescription = new FileInfoMatcherDescription(
				"org.eclipse.ui.ide.multiFilter",
				multiFilterArguments);
