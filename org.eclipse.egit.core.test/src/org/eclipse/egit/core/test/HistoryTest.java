...
public class HistoryTest extends GitTestCase {
	...
	public void queryHistoryThroughTeam() throws IOException, CoreException {
		...
		IDiff diff = subscriber.getDiff(iFile1);
		assertTrue(diff instanceof IThreeWayDiff);
		IThreeWayDiff threeWayDiff = (IThreeWayDiff) diff;
		assertTrue(threeWayDiff.getLocalChange() instanceof ResourceDiff);
		IFileRevision fileRevision = ((ResourceDiff) threeWayDiff
				.getLocalChange()).getBeforeState();
		assertNotNull(fileRevision);

		/*
		 * This is the "before" state of a three-way diff, it should thus
		 * correspond to the common ancestor of the current "master" and
		 * "branch" commits, i.e. the branching point, masterCommit2. However,
		 * this is a Diff for file 1, which is not present in that commit. Our
		 * common ancestor is then masterCommit1.
		 * 
		 * Note that we cannot compare exactly the file revision with the
		 * expected corresponding commit: author and comment information are
		 * missing.
		 */
		assertEquals(masterCommit1.abbreviate(7).name() + "... (J. Git)",
		...
		IFileRevision[] parents = history.getContributors(fileRevision);
		assertEquals(0, parents.length);

		IFileRevision[] children = history.getTargets(fileRevision);
		List<RevCommit> expectedChildren = new ArrayList<RevCommit>(Arrays.asList(masterCommit3, branchCommit1));
