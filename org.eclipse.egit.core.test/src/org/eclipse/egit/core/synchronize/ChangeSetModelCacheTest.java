...
public class ChangeSetModelCacheTest extends LocalDiskRepositoryTestCase {
	...
	public void shouldListOneEmptyCommit() throws Exception {
		...
		assertEmptyCommit(leftResult.get(0), "second commit", LEFT);

		// right asserts
		assertThat(rightResult, notNullValue());
		assertThat(rightResult.size(), is(1));
		assertEmptyCommit(rightResult.get(0), "second commit", RIGHT);
	}
	...
	public void shouldListTwoEmptyCommits() throws Exception {
		// given
		Git git = new Git(db);
		commit(git, "second commit");
		RevCommit c = commit(git, "third commit");
		...
		assertEmptyCommit(leftResult.get(0), "third commit", LEFT);
		assertEmptyCommit(leftResult.get(1), "second commit", LEFT);

		assertThat(rightResult, notNullValue());
		assertThat(rightResult.size(), is(2));
		assertEmptyCommit(rightResult.get(0), "third commit", RIGHT);
		assertEmptyCommit(rightResult.get(1), "second commit", RIGHT);
	}
	...
	public void shouldListAdditionOrDeletionInCommit() throws Exception {
		...
		// left asserts
		assertThat(leftResult, notNullValue());
		assertCommit(leftResult.get(0), "first commit", 1);
		...
		assertThat(rightResult, notNullValue());
		assertCommit(rightResult.get(0), "first commit", 1);
	...
	public void shouldListChangeInCommit() throws Exception {
		...
		// left asserts
		assertThat(leftResult, notNullValue());
		assertCommit(leftResult.get(0), "second commit", 1);
		assertFileChange(leftResult.get(0).getChildren()[0], "a.txt", LEFT | CHANGE);

		// right asserts
		assertThat(rightResult, notNullValue());
		assertCommit(rightResult.get(0), "second commit", 1);
	...
	public void shouldListAllTypeOfChangesInOneCommit() throws Exception {
		...
		// left asserts
		assertThat(leftResult, notNullValue());
		assertCommit(leftResult.get(0), "second commit", 3);
		...
		// right asserts
		assertThat(rightResult, notNullValue());
		assertCommit(rightResult.get(0), "second commit", 3);
	...
	private void assertEmptyCommit(Commit commit, String commitMsg, int direction) {
		assertThat(commit.getShortMessage(), is(commitMsg));
		assertThat(commit.getChildren(), nullValue());
		assertThat(commit.getDirection(), is(direction));
	}

	private void assertCommit(Commit commit, String commitMsg, int childrenCount) {
		assertThat(commit.getShortMessage(), is(commitMsg));
