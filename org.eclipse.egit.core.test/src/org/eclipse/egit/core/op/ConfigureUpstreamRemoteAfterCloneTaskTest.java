...
public class ConfigureUpstreamRemoteAfterCloneTaskTest {
	...
	@Test
	public void testGitHubApiUrl_PublicCloud() {
		assertEquals("https://api.github.com/repos/upstream/repo", //$NON-NLS-1$
				ConfigureUpstreamRemoteAfterCloneTask.gitHubApiUrl(
						"github.com", "upstream/repo")); //$NON-NLS-1$ //$NON-NLS-2$
	}

	@Test
	public void testGitHubApiUrl_Enterprise() {
		assertEquals(
				"https://github.example.com/api/v3/repos/upstream/repo", //$NON-NLS-1$
				ConfigureUpstreamRemoteAfterCloneTask.gitHubApiUrl(
						"github.example.com", "upstream/repo")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	@Test
	public void testGitLabApiUrl_UsesCloneHost() {
		assertEquals(
				"https://gitlab.example.com/api/v4/projects/upstream%2Frepo", //$NON-NLS-1$
				ConfigureUpstreamRemoteAfterCloneTask.gitLabApiUrl(
						"gitlab.example.com", "upstream%2Frepo")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	@Test
	public void testClearsRememberedForkPushMode() throws Exception {
		File workTree = temporaryFolder.newFolder("repo"); //$NON-NLS-1$
		try (Repository repository = FileRepositoryBuilder
				.create(new File(workTree, ".git"))) { //$NON-NLS-1$
			repository.create();
			IEclipsePreferences prefs = RepositoryUtil.INSTANCE
					.getPreferences();
			String key = RepositoryUtil.INSTANCE
					.getRepositorySpecificPreferenceKey(repository,
							ConfigureUpstreamRemoteAfterCloneTask.FORK_UPSTREAM_PUSH_MODE_PREF_KEY);
			prefs.put(key, "upstream"); //$NON-NLS-1$
			prefs.flush();

			ConfigureUpstreamRemoteAfterCloneTask
