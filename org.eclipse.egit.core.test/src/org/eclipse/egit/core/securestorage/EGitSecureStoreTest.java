...
public class EGitSecureStoreTest {
	...
	public void testPutUserAndPassword() throws Exception {
		...
		EGitCredentials credentials = new EGitCredentials("agitter", "letmein");
		store.putCredentials(uri, credentials);

		ISecurePreferences node = secureStoreForTest.node("/GIT/" + EncodingUtils.encodeSlashes(uri.toString()));
	...
	public void testGetUserAndPassword() throws Exception {
		...
		EGitCredentials storedCredentials = store.getCredentials(uri);
	...
	public void testPutUserAndPasswordURIContainingUserAndPass() throws Exception {
		EGitCredentials credentials = new EGitCredentials("agitter", "letmein");
		store.putCredentials(new URIish("http://user:pass@testRepo.example.com/testrepo"), credentials);

		ISecurePreferences node = secureStoreForTest.node("/GIT/" + EncodingUtils.encodeSlashes("http://testRepo.example.com/testrepo"));
		assertEquals("agitter", node.get("user", null));
		assertTrue(node.isEncrypted("password"));
		assertEquals("letmein", node.get("password", null));
	}
	...
	public void testGetUserAndPasswordURIContainingUserAndPass() throws Exception {
		store.putCredentials(new URIish("http://testRepo.example.com/testrepo"), new EGitCredentials("agitter", "letmein"));
		EGitCredentials credentials = store.getCredentials(new URIish("http://agitter:letmein@testRepo.example.com/testrepo"));
	...
	private void setupNewSecureStore() throws IOException, MalformedURLException {
		...
		String secureStorePath = ResourcesPlugin.getWorkspace().getRoot().getLocation().append("testSecureStore").toOSString();
