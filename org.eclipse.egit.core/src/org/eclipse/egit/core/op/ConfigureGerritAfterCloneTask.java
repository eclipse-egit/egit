...
public class ConfigureGerritAfterCloneTask implements PostCloneTask {
	...
	private boolean isGerrit() throws MalformedURLException, IOException,
		...
		if (u.getHost().equals(GIT_ECLIPSE_ORG)) {
			if (s.equals(HTTPS) && (u.getPort() == 443 || u.getPort() == -1)
				&& u.getPath().startsWith(GERRIT_CONTEXT_ROOT)) {
				return true;
			} else if (s.equals(SSH) && (u.getPort() == 29418)) {
				return true;
			}
		}
				...
				while (true) {
					...
					try {
						httpConnection = (HttpURLConnection) new URL(baseURL
								+ path + GERRIT_CONFIG_SERVER_VERSION_API)
								.openConnection();
