...
public final class RemoteBranchInput {
	...
	public static RemoteBranchInput parse(String input) {
		...
		for (ServerType type : ServerType.values()) {
			BranchRef ref = type.parseBranchUrl(trimmed);
			if (ref != null) {
				return new RemoteBranchInput(ref.getBranchName(),
						ref.getOwner(), ref.getServerType());
			}
		}
		...
		if (!trimmed.contains("://")) { //$NON-NLS-1$
			for (ServerType type : ServerType.values()) {
				BranchRef ref = type.parseBranchText(trimmed);
				if (ref != null && Repository.isValidRefName(
						Constants.R_HEADS + ref.getBranchName())) {
					return new RemoteBranchInput(ref.getBranchName(),
							ref.getOwner(), ref.getServerType());
				}
			}
	 ...
	 * @param remotes
	 *            candidates
	 * @return the matching {@link RemoteConfig}, or {@code null} if no match
	 *         is found
	 */
	public RemoteConfig findMatchingRemote(Collection<RemoteConfig> remotes) {
	 ...
	 *         is found
	 */
	public static RemoteConfig findRemoteByOwner(
