 ...
 */
public class StorageContentMerger extends ContentMerger {

	/**
	 * @param db
	 */
	public StorageContentMerger(Repository db) {
		super(db);
	}
		...
		public InputStream getContents() throws CoreException {
			try {
				...
				throw new CoreException(new Status(IStatus.ERROR,
						...
						CoreText.StorageMerger_failedToGetContentForMerge, e));
			}
		}
	...
	public MergeResult<RawText> merge(RawTextComparator cmp,
		...
		try {
			...
			Charset encoding = RawParseUtils.parseEncoding(rawText.getString(0,
					rawText.size(), true).getBytes());
			IStatus merge = instance.merge(output, encoding.name(),
					baseStorage, new EntryStorage(ours), new EntryStorage(
							theirs), null);
			if (merge.isOK()) {
				RawText mergedText = new RawText(output.toByteArray());
				List<RawText> sequences = Collections.singletonList(mergedText);

				mergeResult = new MergeResult<RawText>(sequences);
				mergeResult.add(0, 0, mergedText.size(),
						ConflictState.NO_CONFLICT);
			} else {
				return null;
			}
		} catch (CoreException e) {
			Activator.error(CoreText.StorageContentMerger_ErrorDuringMerge, e);
		}
