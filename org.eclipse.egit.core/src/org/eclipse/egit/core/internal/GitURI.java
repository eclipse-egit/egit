...
public class GitURI {
	...
	/**
	 * Convert the given URI to a GitURI.
	 *
	 * @param uri
	 *            the URI
	 * @return a Git URI
		...
		try {
			if (ProjectSetCapability.SCHEME_SCM.equals(uri.getScheme())) {
				...
				if (ssp.startsWith(SCHEME_GIT)) {
					...
					URIish repository = new URIish(ssp.substring(SCHEME_GIT.length()+1, indexOfSemicolon));
					IPath path = null;
					...
					for (String param : params) {
						if (param.startsWith("path=")) { //$NON-NLS-1$
							path =  new Path(param.substring(param.indexOf('=') + 1));
