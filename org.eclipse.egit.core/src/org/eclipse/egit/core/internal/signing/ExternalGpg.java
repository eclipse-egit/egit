...
class ExternalGpg {
	...
	// Delay the evaluation of isWindows() until getGpg()/getGpgSm() are called
	// the first time.
	private static final class Holder {

		static final String SUFFIX = SystemReader.getInstance().isWindows()
				? ".exe" //$NON-NLS-1$
				: ""; //$NON-NLS-1$
	}

	static String getGpg() {
		return get("gpg" + Holder.SUFFIX); //$NON-NLS-1$
	}

	static String getGpgSm() {
		return get("gpgsm" + Holder.SUFFIX); //$NON-NLS-1$
	}
