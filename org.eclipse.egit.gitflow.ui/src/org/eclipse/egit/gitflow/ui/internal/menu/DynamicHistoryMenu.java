...
public class DynamicHistoryMenu extends ContributionItem {
	...
	public void fill(Menu menu, int index) {
		...
		menuItem.setText("Start GitFlow release from " + abbreviate(selectedCommit) + "..."); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private GitFlowRepository getRepository() {
		IWorkbenchPart activePart = PlatformUI.getWorkbench()
				.getActiveWorkbenchWindow().getActivePage().getActivePart();
		Repository repository = Platform.getAdapterManager().getAdapter(
				activePart, Repository.class);
		if (repository == null) {
			return null;
		}
